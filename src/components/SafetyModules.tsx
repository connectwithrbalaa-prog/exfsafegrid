...
export default function SafetyModules({ customer }: { customer: Customer }) {
  ...
  // Determine default tab — show PSPS first if outage active
  const defaultTab = isOutageActive ? "psps" : "psps";

  return (
    <div className="space-y-4">
      {/* Medical Priority — always visible above tabs */}
      {customer.medical_baseline && isOutageActive && (
        <div className="p-3 rounded-lg border-2 border-destructive/50 bg-destructive/10 space-y-2 shadow-md shadow-destructive/10">
      ...
      <Tabs defaultValue={defaultTab} className="w-full">
        ...
        {/* ===== PSPS EVENT TRACKER TAB ===== */}
        <TabsContent value="psps" className="mt-3">
              ...
              <div className="p-2.5 rounded-md bg-background/60 border border-border space-y-1.5">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    await supabase.from("customers").update({ restoration_timer: val, last_update: new Date().toISOString() } as any).eq("id", customer.id as any);
              ...
              <div className="p-2.5 rounded-md bg-background/60 border border-border space-y-1.5">
                ...
                <input
                  ...
                  onMouseUp={async () => {
                    ...
                    await supabase.from("customers").update({ patrolling_progress: patrolPct, last_update: new Date().toISOString() } as any).eq("id", customer.id as any);
        ...
        {/* ===== CRC TAB ===== */}
        <TabsContent value="crc" className="mt-3">
          {customer.nearest_crc_location ? (
            ...
            <div className="flex items-center justify-center h-24 rounded-lg border border-dashed border-border text-xs text-muted-foreground">
              No CRC assigned for this customer
            </div>
          )}
        </TabsContent>
      </Tabs>
